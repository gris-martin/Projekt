...
def main():
        old = []
        url_g = "http://www.webhallen.com/se-sv/datorkomponenter/fyndvaror/grafikkort/mest_salda/alla/"
        klass_g = [3, 4, 5]
        namn_g = "Grafikkort"

        url_c = "http://www.webhallen.com/se-sv/datorkomponenter/fyndvaror/chassi/mest_salda/alla/"
        klass_c = [3, 4, 5]
        namn_c = "Chassi"

        url_m = 'http://www.webhallen.com/se-sv/datorkomponenter/fyndvaror/minnen/dimm/mest_salda/alla/'
        klass_m = [3, 4, 5]
        namn_m = 'Minne'
        opt_m = 'DDR3'
...
def check_url(url, klass, namn, old_prods, next_page = '', opt = []):
        # Hamta hemsidan (grafikkort)
        print(url+next_page)
        r = request.urlopen(url + next_page)
        
        bytecode = r.read()
        strcode = bytecode.decode(encoding = 'ISO-8859-1')
        
	# Plocka ut alla artiklar och leta efter klasser
        prods = strcode.split('prod_list_row')[1:]
        for prod in prods:
        ...
        page_line = prods[len(prods)-1].split('selected')[1].splitlines()[1]
        if 'list_nav' in page_line:
                next_page = page_line.split('>')[1].split('<')[0]
                check_url(url, klass, namn, old_prods, next_page)
        return
